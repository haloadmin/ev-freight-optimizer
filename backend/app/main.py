...
from .model_runtime import XGBoostJsonRegressor

ROOT = Path(__file__).resolve().parents[2]
BACKEND = ROOT / "backend"
load_dotenv(ROOT / ".env")
MODEL_PATH = BACKEND / "model" / "ev_best_model_compatible.json"
...
MODEL = XGBoostJsonRegressor(MODEL_PATH)
...
def prediction(payload: float) -> tuple[float,float,bool]:
    ...
    base_pred=MODEL.predict(BASE_CONDITION)
